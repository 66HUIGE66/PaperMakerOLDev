...
public class LearningStatisticsTool {
    ...
    public String getSingleSubjectStatistics(@V(value = "当前用户Id") Long userId, 
        ...
        try {
            ...
            System.out.println("获取单学科统计数据成功，用户ID: " + userId + ", 学科: " + subjectName + ", 记录数: " + subjectRecords.size());
            return objectMapper.writeValueAsString(statistics);
        } catch (Exception e) {
    ...
    private Map<String, Object> calculateSingleSubjectStatistics(List<ExamRecordEntity> records, String subjectName) {
        ...
        int completedCount = (int) records.stream().filter(r -> r.getStatus() == ExamRecordEntity.ExamStatus.COMPLETED).count();
        ...
        result.put("questionStatistics", Map.of(
            ...
            "correctRate", totalQuestions > 0 ? String.format("%.2f", (double) totalCorrect / totalQuestions * 100) + "%" : "0%"
        ));
...
//    public String getSingleSubjectKnowledgePointStatistics(@V(value = "当前用户Id") Long userId,
//                                                          @V(value = "学科名称") String subjectName) {
//        try {
//            if (userId == null) {
//                System.out.println("========== 错误：无法获取用户ID ==========");
...
//
//            // 计算知识点统计数据
//            Map<String, Object> statistics = calculateSingleSubjectKnowledgePointStatistics(subjectRecords, subjectName);
//
//            System.out.println("获取单学科知识点统计数据成功，用户ID: " + userId + ", 学科: " + subjectName + ", 记录数: " + subjectRecords.size());
//            return objectMapper.writeValueAsString(statistics);
//        } catch (Exception e) {
...
//     * 参考ExamRecordServiceImpl.getSubjectKnowledgePointStatistics()方法的实现
//     */
//    private Map<String, Object> calculateSingleSubjectKnowledgePointStatistics(List<ExamRecordEntity> records, String subjectName) {
//        Map<String, Object> result = new HashMap<>();
//        result.put("subjectName", subjectName);
//        result.put("hasData", true);
//
...
//
//        // 计算总体统计
//        int totalQuestions = knowledgePoints.stream().mapToInt(kp -> (Integer) kp.get("questionCount")).sum();
//        int totalCorrect = knowledgePoints.stream().mapToInt(kp -> (Integer) kp.get("correctCount")).sum();
//        long totalTime = knowledgePoints.stream().mapToLong(kp -> (Long) kp.get("totalTimeSeconds")).sum();
//        double overallAccuracy = totalQuestions > 0 ? (totalCorrect * 100.0 / totalQuestions) : 0.0;
//
//        result.put("summary", Map.of(
//            "totalKnowledgePoints", knowledgePoints.size(),
//            "totalQuestions", totalQuestions,
