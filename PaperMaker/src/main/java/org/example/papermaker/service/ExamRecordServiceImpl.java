 ...
 */
@Service
public class ExamRecordServiceImpl extends ServiceImpl<ExamRecordMapper, ExamRecordEntity>
        implements ExamRecordService {

    @Override
    public boolean recalculateScore(Long examRecordId) {
        ExamRecordEntity record = getById(examRecordId);
        if (record == null)
            return false;

        List<AnswerRecordEntity> answers = answerRecordService.lambdaQuery()
        ...
        for (AnswerRecordEntity answer : answers) {
            if (answer.getFinalScore() != null) {
                totalScore += answer.getFinalScore();
            } else if (answer.getSimilarityScore() != null) {
                totalScore += answer.getSimilarityScore();
            } else if (Boolean.TRUE.equals(answer.getIsCorrect())) {
                // Get dynamic score from exam paper configuration
                Double questionScore = examPaperService.getQuestionScore(record.getPaperId(), answer.getQuestionId());
                totalScore += questionScore != null ? questionScore : 5.0;
            }
        ...
        record.setAnsweredQuestions(answeredCount);

        if (record.getTotalQuestions() != null && record.getTotalQuestions() > 0) {
            record.setAccuracy((double) correctCount / record.getTotalQuestions() * 100);
        } else if (answeredCount > 0) {
            record.setAccuracy((double) correctCount / answeredCount * 100);
        } else {
    ...
    public List<Map<String, Object>> getWrongQuestions(Long userId) {
        // 1. Get all answer records for this user where isCorrect is false or score <
        // max
        // First get all examRecordIds for this user
        List<ExamRecordEntity> userRecords = getRecordsByUserId(userId);
        if (userRecords.isEmpty()) {
            return new ArrayList<>();
        }

        List<Long> recordIds = userRecords.stream()
                .map(ExamRecordEntity::getId)
                .collect(java.util.stream.Collectors.toList());

        List<AnswerRecordEntity> wrongAnswers = answerRecordService.lambdaQuery()
                .in(AnswerRecordEntity::getExamRecordId, recordIds)
                .and(w -> w.eq(AnswerRecordEntity::getIsCorrect, false)
                        .or()
                        .lt(AnswerRecordEntity::getFinalScore, 5.0)) // Assuming 5 is max score for now
                .list();

        // 2. Group by questionId to deduplicate and count mistakes
        // Map<QuestionId, Count>
        Map<Long, Integer> mistakeCounts = new HashMap<>();
        Map<Long, AnswerRecordEntity> latestMistakeRecord = new HashMap<>();

        for (AnswerRecordEntity answer : wrongAnswers) {
            Long questionId = answer.getQuestionId();
            if (questionId == null)
                continue;

            mistakeCounts.put(questionId, mistakeCounts.getOrDefault(questionId, 0) + 1);

            // Keep the latest answer record for metadata
            if (!latestMistakeRecord.containsKey(questionId) ||
                    (answer.getId() > latestMistakeRecord.get(questionId).getId())) {
                latestMistakeRecord.put(questionId, answer);
            }
        }

        // 3. Fetch Question Details and build result
        List<Map<String, Object>> result = new ArrayList<>();
        Set<Long> questionIds = mistakeCounts.keySet();
        ...
        if (!questionIds.isEmpty()) {
            ...
            for (Long qId : questionIds) {
                ...
                item.put("question", q);
                item.put("mistakeCount", mistakeCounts.get(qId));
                AnswerRecordEntity latest = latestMistakeRecord.get(qId);
                item.put("lastMistakeTime", latest != null ? latest.getCreatedAt() : null);
                item.put("myLastAnswer", latest != null ? latest.getUserAnswer() : null);
                item.put("aiFeedback", latest != null ? latest.getAiFeedback() : null);
                item.put("aiSuggestions", latest != null ? latest.getAiSuggestions() : null);

                // Get Subject Name
                if (q.getSubjectId() != null) {
                    try {
                        SubjectEntity subject = subjectService.getById(q.getSubjectId());
                        item.put("subjectName", subject != null ? subject.getName() : "未知学科");
                    } catch (Exception e) {
                        item.put("subjectName", "未知学科");
                    }
                } else {
                    item.put("subjectName", "未知学科");
                }
        ...
        result.sort((a, b) -> ((Integer) b.get("mistakeCount")).compareTo((Integer) a.get("mistakeCount")));
    ...
    public Map<String, Object> getOverallStatistics(Long userId) {
        ...
        double maxScore = scores.isEmpty() ? 0 : scores.stream().mapToDouble(Double::doubleValue).max().orElse(0);
        double minScore = scores.isEmpty() ? 0 : scores.stream().mapToDouble(Double::doubleValue).min().orElse(0);
        ...
        double maxAccuracy = accuracies.isEmpty() ? 0.0
                : accuracies.stream().mapToDouble(Double::doubleValue).max().orElse(0.0);
        double minAccuracy = accuracies.isEmpty() ? 0.0
        ...
        statistics.put("questionStatistics", Map.of(
                ...
                "answerRate",
                totalQuestions > 0 ? String.format("%.2f", (double) totalAnswered / totalQuestions * 100) + "%" : "0%",
                "correctRate",
                totalAnswered > 0 ? String.format("%.2f", (double) totalCorrect / totalAnswered * 100) + "%" : "0%"));

        statistics.put("typeStatistics", Map.of(
                "practiceCount", practiceCount,
                "examCount", examCount,
                "practicePercentage",
                totalRecords > 0 ? String.format("%.2f", (double) practiceCount / totalRecords * 100) + "%" : "0%",
                "examPercentage",
                totalRecords > 0 ? String.format("%.2f", (double) examCount / totalRecords * 100) + "%" : "0%"));
    ...
    public Map<String, Object> getSubjectStatistics(Long userId) {
        ...
        for (ExamRecordEntity record : records) {
            ...
            if (record.getScore() != null) {
                stats.put("totalScore", (Double) stats.get("totalScore") + record.getScore());
                ((List<Double>) stats.get("scoreList")).add(record.getScore());
            }
        ...
        for (Map.Entry<String, Map<String, Object>> entry : subjectStats.entrySet()) {
            ...
            double totalScore = (Double) stats.get("totalScore");
            ...
            @SuppressWarnings("unchecked")
            List<Double> scoreList = (List<Double>) stats.get("scoreList");
            ...
            double avgScore = scoreList.isEmpty() ? 0.0
                    : scoreList.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);
            double maxScore = scoreList.isEmpty() ? 0
                    : scoreList.stream().mapToDouble(Double::doubleValue).max().orElse(0);
            double minScore = scoreList.isEmpty() ? 0
                    : scoreList.stream().mapToDouble(Double::doubleValue).min().orElse(0);
    ...
    public Map<String, Object> getSubjectKnowledgePointStatistics(Long userId) {
        ...
        for (Map.Entry<String, Map<String, Map<String, Object>>> subjectEntry : subjectKpStats.entrySet()) {
            ...
            for (Map.Entry<String, Map<String, Object>> kpEntry : kpStatsMap.entrySet()) {
                ...
                kpData.put("averageTimeFormatted",
                        formatTime(questionCount > 0 ? totalTimeSeconds / questionCount : 0));
    ...
    public boolean createRecord(ExamRecordEntity record) {
        try {
            ...
            if (record.getTimeSpent() == null && record.getStartTime() != null && record.getEndTime() != null) {
                long diffInSeconds = java.time.Duration.between(record.getStartTime(), record.getEndTime())
                        .getSeconds();
