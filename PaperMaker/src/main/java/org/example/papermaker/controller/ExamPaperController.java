...
public class ExamPaperController {
    ...
    public RespBean getPapersPage(
        ...
        System.out.println("参数 - current: " + current + ", size: " + size + ", searchText: " + searchText + ", type: "
        ...
        try {
            ...
            System.out.println("当前用户信息: "
                    + (currentUser != null
                            ? "ID=" + currentUser.getId() + ", 用户名=" + currentUser.getUsername() + ", 角色="
                                    + currentUser.getRole()
                            : "null"));
            ...
            if (!SimplePermissionUtils.isAdmin(currentUser)) {
                ...
                System.out.println(
                        "查询条件：系统试卷（isSystem=true）或用户创建的试卷（isSystem=false AND creatorId=" + currentUser.getId() + "）");
            ...
            // 类型筛选（注意：generationType 不是数据库字段，无法用于lambda查询）
            // 如果需要类型筛选，需要添加数据库字段或在内存中过滤
            ...
            if (subjectId != null) {
                ...
                System.out.println("筛选前的试卷ID列表: "
                        + result.getRecords().stream().map(ExamPaperEntity::getId).collect(Collectors.toList()));
                ...
                System.out.println("筛选后的试卷ID列表: "
                        + filteredRecords.stream().map(ExamPaperEntity::getId).collect(Collectors.toList()));
            ...
            List<Map<String, Object>> enrichedRecords = filteredRecords.stream().map(paper -> {
                ...
                // 推断 generationType
                String genType = paper.getGenerationType();
                if (genType == null || genType.isEmpty()) {
                    genType = (paper.getRuleId() != null && paper.getRuleId() > 0) ? "AUTO" : "MANUAL";
                }
                paperMap.put("generationType", genType);
                ...
                System.out.println("试卷ID: " + paper.getId() + ", creatorId: " + paper.getCreatorId() + ", creatorName: "
                        + creatorName);
    ...
    public RespBean getPaperById(
            @Parameter(description = "试卷ID", required = true, example = "1") @PathVariable Long id) {
        // 权限检查：获取当前用户
        UserEntity currentUser = SimpleUserContext.getCurrentUser();
        if (!SimplePermissionUtils.hasViewPermission(currentUser)) {
            return new RespBean(403, "权限不足", null);
    ...
    public RespBean getPapersByCreator(
            @Parameter(description = "创建者ID", required = true, example = "1") @PathVariable Long creatorId) {
    ...
    public RespBean createPaper(
            @Parameter(description = "试卷信息", required = true) @RequestBody ExamPaperEntity paper) {
        ...
        logger.info("创建试卷 - 标题: {}, questionIds: {}, totalScore: {}, duration: {}, subjectId: {}",
                paper.getTitle(), paper.getQuestionIds(), paper.getTotalScore(), paper.getDuration(),
                paper.getSubjectId());
                ...
                try {
                    ...
                    for (int i = 0; i < questionIdArray.length; i++) {
                        ...
                        try {
                            ...
                            boolean added = examPaperService.addQuestionToPaper(paper.getId(), questionId,
                                    questionScore);
    ...
    public RespBean updatePaper(
        ...
        if (success) {
            ...
            // 推断 generateType
            String genType = updatedPaper.getGenerationType();
            if (genType == null || genType.isEmpty()) {
                genType = (updatedPaper.getRuleId() != null && updatedPaper.getRuleId() > 0) ? "AUTO" : "MANUAL";
            }
            vo.setGenerationType(genType);
    ...
    public RespBean copySystemPaper(
            @Parameter(description = "试卷ID", required = true, example = "1") @PathVariable Long paperId) {
